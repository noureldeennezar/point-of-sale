...
class ShiftService {
  ...
  Future<void> fixExistingShift(String shiftId, String storeId) async {
    try {
      await _firestore
          .collection('stores')
          .doc(storeId)
  ...
  Future<String?> openShift(String storeId) async {
    ...
    try {
      if (await isOnline()) {
        DocumentReference shiftRef = await _firestore
            .collection('stores')
            .doc(storeId)
            .collection('shifts')
            .add(shiftData);
  ...
  Future<bool> closeShift(String shiftId, String storeId, {double? finalCashCount}) async {
    final updateData = {
  ...
  Future<String?> addTransaction({
    ...
    try {
      if (await isOnline()) {
        DocumentReference txnRef = await _firestore
            .collection('stores')
            .doc(storeId)
  ...
  Future<void> syncOfflineShifts(String storeId) async {
    ...
    if (await isOnline()) {
      ...
      final queueItems =
      await db.query('offline_queue', where: 'store_id = ?', whereArgs: [storeId]);
      for (var item in queueItems) {
        ...
        switch (item['action']) {
          case 'open_shift':
            batch.set(
                _firestore.collection('stores').doc(storeId).collection('shifts').doc(itemId),
                data);
            break;
          case 'close_shift':
            batch.update(
          ...
          case 'add_transaction':
            ...
            batch.set(
                ...
                data);
            batch.update(
                _firestore.collection('stores').doc(storeId).collection('shifts').doc(shiftId),
                {
                  'transactions': FieldValue.arrayUnion([itemId]),
                });
            break;
        }
        await db.delete('offline_queue', where: 'id = ? AND store_id = ?', whereArgs: [itemId, storeId]);
      }
      try {
        await batch.commit();
        if (_context != null && _context!.mounted) {
  ...
  Future<List<Map<String, dynamic>>> getShifts({
    ...
    try {
      Query query = _firestore.collection('stores').doc(storeId).collection('shifts');
      if (userId != null) {
  ...
  Future<List<Map<String, dynamic>>> getShiftTransactions(String shiftId, String storeId) async {
    try {

...
class _OrdersTableState extends State<OrdersTable> {
  ...
  Future<void> loadOrders() async {
    ...
    try {
      List<Map<String, dynamic>> data = await dbHelper.fetchOrdersWithItems(storeId);
      if (data.isEmpty) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
      ...
      for (var order in data) {
        ...
        orderMap[orderNumber]!['totalPrice'] += (totalPrice * 1.2); // Assuming 20% tax/service
      }
  ...
  Future<void> deleteItem(String itemCode, int orderNumber) async {
    ...
    ItemDatabaseHelper dbHelper = ItemDatabaseHelper();
    try {
      await dbHelper.deleteItem(itemCode, storeId);
      await loadOrders();
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
  ...
  Future<void> clearDatabase() async {
    ...
    ItemDatabaseHelper dbHelper = ItemDatabaseHelper();
    try {
      await dbHelper.clearDatabase(storeId);
      await loadOrders();
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
  ...
  Widget build(BuildContext context) {
    ...
    return StreamBuilder<Map<String, dynamic>?>(
      stream: Provider.of<AuthService>(context, listen: false).currentUserWithRole,
